...
func main() {
	...
	request := &vectors.GetBucketResourceGroupRequest{
		Bucket: oss.Ptr(bucketName),
	}
	result, err := client.GetBucketResourceGroup(context.TODO(), request)
	if err != nil {
		log.Fatalf("failed to get vector bucket resource group %v", err)
	}

	log.Printf("get vector bucket resource group result:%#v\n", result)
}
