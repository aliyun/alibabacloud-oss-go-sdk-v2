...
func main() {
	...
	request := &agentic.DeleteAgenticBucketRequest{
		Bucket: oss.Ptr(bucket),
	}
	result, err := client.DeleteAgenticBucket(context.TODO(), request)
	if err != nil {
		log.Fatalf("failed to delete agentic bucket %v", err)
	}
	log.Printf("delete agentic bucket result:%#v\n", result)
}
