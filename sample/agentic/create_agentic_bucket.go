...
func main() {
	...
	request := &agentic.CreateAgenticBucketRequest{
		Bucket: oss.Ptr(bucket),
		CreateAgenticBucketConfiguration: &agentic.CreateAgenticBucketConfiguration{
			StorageClass:       oss.StorageClassStandard,
			DataRedundancyType: oss.DataRedundancyLRS,
		},
	}
	result, err := client.CreateAgenticBucket(context.TODO(), request)
	if err != nil {
		log.Fatalf("failed to create agentic bucket %v", err)
	}
	log.Printf("create agentic bucket result:%#v\n", result)
}
