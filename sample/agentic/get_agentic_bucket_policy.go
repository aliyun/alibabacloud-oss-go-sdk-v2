...
func main() {
	...
	request := &agentic.GetAgenticBucketPolicyRequest{
		Bucket: oss.Ptr(bucket),
	}
	result, err := client.GetAgenticBucketPolicy(context.TODO(), request)
	if err != nil {
		log.Fatalf("failed to get agentic bucket policy %v", err)
	}
	log.Printf("get agentic bucket policy result: policy:%v\n", result.Body)
}
