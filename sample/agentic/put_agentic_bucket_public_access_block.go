...
func main() {
	...
	request := &agentic.PutAgenticBucketPublicAccessBlockRequest{
		Bucket: oss.Ptr(bucket),
		PublicAccessBlockConfiguration: &agentic.PublicAccessBlockConfiguration{
			BlockPublicAccess: oss.Ptr(true),
		},
	}
	result, err := client.PutAgenticBucketPublicAccessBlock(context.TODO(), request)
	if err != nil {
		log.Fatalf("failed to put agentic bucket public access block %v", err)
	}
	log.Printf("put agentic bucket public access block result:%#v\n", result)
}
