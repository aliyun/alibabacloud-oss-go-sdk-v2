...
func main() {
	...
	request := &agentic.PutAgenticBucketEncryptionRequest{
		Bucket: oss.Ptr(bucket),
		ServerSideEncryptionRule: &agentic.ServerSideEncryptionRule{
			ApplyServerSideEncryptionByDefault: &agentic.ApplyServerSideEncryptionByDefault{
				SSEAlgorithm: oss.Ptr("AES256"),
			},
		},
	}
	result, err := client.PutAgenticBucketEncryption(context.TODO(), request)
	if err != nil {
		log.Fatalf("failed to put agentic bucket encryption %v", err)
	}
	log.Printf("put agentic bucket encryption result:%#v\n", result)
}
