...
func main() {
	...
	request := &agentic.GetAgenticBucketPublicAccessBlockRequest{
		Bucket: oss.Ptr(bucket),
	}
	result, err := client.GetAgenticBucketPublicAccessBlock(context.TODO(), request)
	if err != nil {
		log.Fatalf("failed to get agentic bucket public access block %v", err)
	}

	if result.PublicAccessBlockConfiguration != nil {
		log.Printf("get agentic bucket public access block result: block public access:%v\n",
			oss.ToBool(result.PublicAccessBlockConfiguration.BlockPublicAccess))
	}
}
