...
func main() {
	...
	request := &agentic.DeleteAgenticBucketEncryptionRequest{
		Bucket: oss.Ptr(bucket),
	}
	result, err := client.DeleteAgenticBucketEncryption(context.TODO(), request)
	if err != nil {
		log.Fatalf("failed to delete agentic bucket encryption %v", err)
	}
	log.Printf("delete agentic bucket encryption result:%#v\n", result)
}
