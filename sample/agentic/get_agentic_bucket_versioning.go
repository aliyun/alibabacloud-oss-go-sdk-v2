...
func main() {
	...
	request := &agentic.GetAgenticBucketVersioningRequest{
		Bucket: oss.Ptr(bucket),
	}
	result, err := client.GetAgenticBucketVersioning(context.TODO(), request)
	if err != nil {
		log.Fatalf("failed to get agentic bucket versioning %v", err)
	}

	if result.VersioningConfiguration != nil {
		log.Printf("get agentic bucket versioning result: status:%v\n", result.VersioningConfiguration.Status)
	}
}
