...
func init() {
	...
	flag.StringVar(&acl, "acl", "private", "The access control list (ACL) of the agentic bucket, e.g. private, public-read.")
}
...
func main() {
	...
	request := &agentic.PutAgenticBucketAclRequest{
		Bucket: oss.Ptr(bucket),
		Acl:    oss.BucketACLType(acl),
	}
	result, err := client.PutAgenticBucketAcl(context.TODO(), request)
	if err != nil {
		log.Fatalf("failed to put agentic bucket acl %v", err)
	}
	log.Printf("put agentic bucket acl result:%#v\n", result)
}
