...
func main() {
	...
	request := &agentic.GetAgenticBucketAclRequest{
		Bucket: oss.Ptr(bucket),
	}
	result, err := client.GetAgenticBucketAcl(context.TODO(), request)
	if err != nil {
		log.Fatalf("failed to get agentic bucket acl %v", err)
	}
	log.Printf("get agentic bucket acl result: acl:%v\n", oss.ToString(result.ACL))
}
