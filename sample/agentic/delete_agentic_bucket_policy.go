...
func main() {
	...
	request := &agentic.DeleteAgenticBucketPolicyRequest{
		Bucket: oss.Ptr(bucket),
	}
	result, err := client.DeleteAgenticBucketPolicy(context.TODO(), request)
	if err != nil {
		log.Fatalf("failed to delete agentic bucket policy %v", err)
	}
	log.Printf("delete agentic bucket policy result:%#v\n", result)
}
