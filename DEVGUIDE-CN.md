...
该开发套件同时提供实用的组件，例如上传和下载管理器，自动将大对象分成多块并行传输。
...
> **Note:**
> 生成凭证的命令不可由未经批准的进程或用户访问，则可能存在安全风险。
> 生成凭证的命令不会把任何秘密信息写入 stderr 或 stdout，因为该信息可能会被捕获或记录，可能会将其向未经授权的用户公开。
...
本部分介绍如何SDK提供的接口和组件, 以及如何使用这些接口和组件。
...
err = bucket.PutObject("exampledir/exampleobject.txt", bytes.NewReader([]byte("example data")))
```
...
V2版本下， '\<OperationName\>Request' 要求输入参数需要通过指针方式传递给操作接口，同时 '\<OperationName\>Result' 把返回参数通过指针方式返回给调用者。
